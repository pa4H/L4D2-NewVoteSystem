...
new Handle:g_AllTalkCvar; // Переменная для взаимодействия с sv_alltalk
...
new String:argOne[MAXPLAYERS + 1][128]; // Первый аргумент из консоли
new String:argTwo[MAXPLAYERS + 1][128]; // Второй аргумент из консоли

new voteInTeam; // Сюда пишется номер тимы где будет происходить голосование
new String:callerName[MAXPLAYERS + 1][32]; // Имя игрока начавшего голосование
new String:voteName[128]; // Название голсования
new String:votePassAnswer[128]; // Ответ на состоявшееся голосование (текст)
new String:buferArgument[128]; // Буферная переменная для хранения 1 аргумента
new String:buferArgument2[128]; // Буферная переменная для хранения 2 аргумента
char txtBufer[192]; // Буферная переменная для форматирования текста
...
public void OnPluginStart()
{
	...
	LoadTranslations("NewVoteSystem.phrases"); // Загружаем тексты всех фраз
	LoadTranslations("NewVoteSystemMaps.phrases"); // Загружаем названия карт
...
public Action kickSpecVote(int client, int args)
{
	if (!VoteInProgress)
	{
		Format(argOne[client], sizeof(argOne[]), "KickSpec"); // Вписываем в аргумент1 название кастомного голосования
		createVote(client, L4D2_TEAM_ALL);
...
public Action kickInfectedBotsVote(int client, int args)
{
	if (!VoteInProgress)
	{
		...
		{
			Format(argOne[client], sizeof(argOne[]), "KillInfectedBots"); // Вписываем в аргумент1 название кастомного голосования
...
public void createVote(int client, int team)
{
	...
	UpdateVotes();
	g_hTimer = CreateTimer(10.0, Timer_VoteCheck, TIMER_FLAG_NO_MAPCHANGE); // Спустя это время голосование закончится
}
...
void votePassedFunc() // Если голосование успешно, то выполняем...
{
	...
	{
		L4D2_ChangeLevel(buferArgument2); // При помощи native void L4D2_ChangeLevel меняем карту.
	}
	...
	if (StrEqual(buferArgument, "ChangeMission", false)) // В качестве аргумента приводится L4D2C8 вместо c8m1_apartments
	{
		...
		FormatEx(buferArgument2, sizeof(buferArgument2), "%t", buf); // Даём L4D2C1, получаем c1m1_hotel
		L4D2_ChangeLevel(buferArgument2); // При помощи native void L4D2_ChangeLevel меняем карту.
	}
	...
	{
		char current_map[53];
		GetCurrentMap(current_map, sizeof(current_map)); // Получаем называние карты (c8m1_apartments)
		L4D2_ChangeLevel(current_map); // Меняем на ту же самую карту
	}
}
